...
def u_net_bat_nor(x, is_train=False, reuse=False, n_out=1):
    _, nx, ny, nz = x.get_shape().as_list()
    print(" * Input: size of image: %d %d %d" %(nx, ny, nz))
    w_init = tf.truncated_normal_initializer(stddev=0.01)
    b_init = tf.truncated_normal_initializer(stddev=0)
    ...
    with tf.variable_scope("u_net", reuse=reuse):
        ...
        conv1 = Conv2d(inputs, 64, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv1_1')
        conv1 = BatchNormLayer(conv1, act=lambda x: tl.act.lrelu(x, 0.2), is_train=is_train, gamma_init=gamma_init, name='bn1_1')
        conv1 = Conv2d(conv1, 64, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv1_2')
        conv1 = BatchNormLayer(conv1,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn1_2')
        pool1 = MaxPool2d(conv1, (2, 2), name='pool1')

        conv2 = Conv2d(pool1, 128, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv2_1')
        conv2 = BatchNormLayer(conv2,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn2_1')
        conv2 = Conv2d(conv2, 128, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv2_2')
        conv2 = BatchNormLayer(conv2,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn2_2')
        pool2 = MaxPool2d(conv2, (2, 2), name='pool2')

        conv3 = Conv2d(pool2, 256, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv3_1')
        conv3 = BatchNormLayer(conv3,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn3_1')
        conv3 = Conv2d(conv3, 256, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv3_2')
        conv3 = BatchNormLayer(conv3,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn3_2')
        pool3 = MaxPool2d(conv3, (2, 2), name='pool3')

        conv4 = Conv2d(pool3, 512, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv4_1')
        conv4 = BatchNormLayer(conv4,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn4_1')
        conv4 = Conv2d(conv4, 512, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv4_2')
        conv4 = BatchNormLayer(conv4,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn4_2')
        pool4 = MaxPool2d(conv4, (2, 2), name='pool4')

        conv5 = Conv2d(pool4, 1024, (3, 3), act=None, W_init=w_init, b_init=b_init, name='conv5_1')
        conv5 = BatchNormLayer(conv5,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='bn5')
        conv5 = Conv2d(conv5, 1024, (3, 3), act=lambda x: tl.act.lrelu(x,0.2), W_init=w_init, b_init=b_init, name='conv5_2')

        up4 = DeConv2d(conv5, 512, (3, 3), (nx/8, ny/8), (2, 2), act=None, W_init=w_init, b_init=b_init, name='deconv4')
        up4 = BatchNormLayer(up4,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='up4')
        up4 = ConcatLayer([up4, conv4], 3, name='concat4')

        conv4 = Conv2d(up4, 512, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv4_1')
        conv4 = BatchNormLayer(conv4,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn4_1')
        conv4 = Conv2d(conv4, 512, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv4_2')
        conv4 = BatchNormLayer(conv4,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn4_2')

        up3 = DeConv2d(conv4, 256, (3, 3), (nx/4, ny/4), (2, 2), act=None, W_init=w_init, b_init=b_init, name='deconv3')
        up3 = BatchNormLayer(up3,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='up3')
        up3 = ConcatLayer([up3, conv3], 3, name='concat3')

        conv3 = Conv2d(up3, 256, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv3_1')
        conv3 = BatchNormLayer(conv3,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn3_1')
        conv3 = Conv2d(conv3, 256, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv3_2')
        conv3 = BatchNormLayer(conv3,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn3_2')

        up2 = DeConv2d(conv3, 128, (3, 3), (nx/2, ny/2), (2, 2), act=None, W_init=w_init, b_init=b_init, name='deconv2')
        up2 = BatchNormLayer(up2,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='up2')
        up2 = ConcatLayer([up2, conv2], 3, name='concat2')

        conv2 = Conv2d(up2, 128, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv2_1')
        conv2 = BatchNormLayer(conv2,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn2_1')
        conv2 = Conv2d(conv2, 128, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv2_2')
        conv2 = BatchNormLayer(conv2,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn2_2')

        up1 = DeConv2d(conv2, 64, (3, 3), (nx/1, ny/1), (2, 2), act=None, W_init=w_init, b_init=b_init, name='deconv1')
        up1 = BatchNormLayer(up1,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='up1')
        up1 = ConcatLayer([up1, conv1] , 3, name='concat1')

        conv1 = Conv2d(up1, 64, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv1_1')
        conv1 = BatchNormLayer(conv1,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn1_1')
        conv1 = Conv2d(conv1, 64, (3, 3), act=None, W_init=w_init, b_init=b_init, name='uconv1_2')
        conv1 = BatchNormLayer(conv1,act=lambda x: tl.act.lrelu(x,0.2),is_train=is_train,gamma_init=gamma_init,name='ubn1_2')
        conv1 = Conv2d(conv1, n_out, (1, 1), act=tf.nn.sigmoid, W_init=w_init, b_init=b_init, name='uconv1')
    return conv1
...
def get_cost(logits, labels,num_class,cost_name):
    """
    ...
    flat_labels = tf.reshape(labels, [-1, num_class])
