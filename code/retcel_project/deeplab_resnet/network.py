...
class Network(object):
    ...
    def feed(self, *args):
        ...
        for fed_layer in args:
            if isinstance(fed_layer, np.compat.basestring):
                try:
    ...
    def make_var_bias(self, name, shape):
        '''Creates a new TensorFlow variable.'''
        return tf.get_variable(name, shape, initializer=tf.constant_initializer(0.1), trainable=self.trainable)
    ...
    def conv(self,
        ...
        with tf.variable_scope(name) as scope:
            ...
            # Add the biases
            if biased:
                biases = self.make_var_bias('biases', [c_o])
    ...
    def deconv(self,
        ...
        c_i = input.get_shape()[-1]
        deconvolve = lambda i, k: tf.nn.conv2d_transpose(i, k, tf.stack([x_shape[0], out_shape[0], out_shape[1], c_o]),
...
if __name__ =='__main__':
    inputs=tf.placeholder(tf.float32,shape=[10,64,64,1])
    net=Network({'image_inputs':inputs})
