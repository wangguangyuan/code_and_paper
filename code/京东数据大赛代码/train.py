...
def xgboost_make_submission():
    ...
    param = {'learning_rate' : 0.08, 'n_estimators':1000 , 'max_depth': 3, 
        'min_child_weight': 8, 'gamma': 0.2, 'subsample': 0.8, 'colsample_bytree': 0.8,
        'scale_pos_weight': 0.5, 'eta': 0.1, 'silent': 1, 'objective': 'binary:logistic'}
    num_round =400
    param['nthread'] = 4
    #param['eval_metric'] = "auc"
    plst =list(param.items())
    plst += list([('eval_metric', 'logloss')])
